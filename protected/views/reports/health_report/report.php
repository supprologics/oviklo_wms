                ...
                <table id="popularity" class="table data reports_table table-sm table-bordered" cellspacing="0" cellpadding="0" width="100%">
                    ...
                    <tbody>
                        ...
                        $stocklot = Yii::app()->db->createCommand("SELECT "
                                        . "SUM(qty) as tot,DATEDIFF(expire_date,'". date("Y-m-d") ."') as health,"
                                        . "warehouse_id,project_id,"
                                        . "batch_no,manf_date,expire_date,"
                                        . "sku_id,"
                                        . "goods_sts_id "
                                        . "FROM stock WHERE "
                                        . "customers_id = '" . $customer->id . "' AND tbl_name IN ('grn','pick','tn') $warehouse_id $project_id $goodsSts_id $sku_q "
                                        . "GROUP BY "
                                        . "goods_sts_id,sku_id,warehouse_id,project_id,batch_no HAVING tot != 0 ORDER BY sku_id ASC")->queryAll();
                        ...
                        foreach ($stocklot as $value) {
                            ...
                            <tr>
                                ...
                                <td class="text-right"><?php echo $value['health']; ?></td> 
                            </tr>
