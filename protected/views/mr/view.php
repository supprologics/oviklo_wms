<script>
    ...
    function save () {
       ...
       $.ajax ({
          url: "<?php echo Yii::app()->createUrl("mrItems/updateAll/" . $model->id) ?>",
          data: inner_data,
          type: "POST",
          success: showResponse,
          error: showResponse
    ...
    $ (document).on ("click", "#Mr-delete", function (e) {
       ...
       if (confirmdata == true) {
          $.ajax ({
             ...
             window.location.href = "<?php echo Yii::app()->createUrl('Mr') ?>";
          });
       }
    });
    function search () {
    ...
    $ (document).on ("click", "#mrItems-remove", function (e) {
       ...
       do {
          ...
          $.ajax ({
             url: "<?php echo Yii::app()->createUrl('mrItems/delete') ?>/" + val[i],
             success: showResponse,
             type: "POST",
             async: false,
             error: showResponse,
    ...
    $ (document).on ("click", "#Mr-complete", function (e) {
       ...
       if (confirmdata == true) {
          ...
          if (sts > 0) {
             window.location.href = "<?php echo Yii::app()->createUrl('Mr') ?>";
             window.open ("<?php echo Yii::app()->createUrl('Mr/print/') ?>/" + id, "mywindow", "location=1,status=1,scrollbars=1, width=800,height=600");
          }
       }
    });
...
<div id="form_body">
    <div class="container-fluid">
        ...
        <h2>Material Request - <?php echo $model->code; ?></h2>
        <div class="row">
            <div class="col-sm-5">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-5">
                        <input type="text" name="delivery_date" id="delivery_date" value="<?php echo $model->delivery_date; ?>" class="form-control datepicker form-control-sm" />
                    </div>
                </div>
                <div class="form-group row">
            ...
            <div class="modal-dialog  modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="cus-form">
                            <form enctype="multipart/form-data" class="form-horizontal" action="<?php echo Yii::app()->createUrl('mrItems/csvupload') ?>" method="post" id="Grn-csv">
        ...
        <hr />
        <div id="inline_manu">
            <button data-toggle="modal" data-target="#MrItems-addmodel" class="btn btn-success btn-sm">Add SKUs <span class="oi oi-plus"></span></button>
        ...
        <div id="btn_bar" class="mt-2 text-right">
            <div class="row">
                <div class="col">
                    <button id="Mr-delete" data-id="<?php echo $model->id; ?>" class="btn btn-sm btn-danger"> Revoke MR <span class="oi oi-x"></span></button>
                    <button id="Mr-complete" data-id="<?php echo $model->id; ?>" class="btn  btn-sm btn-success">Complete MR</button>
                </div>
            </div>
        </div>
