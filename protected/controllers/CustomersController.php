...
class CustomersController extends Controller {
    ...
    public function actionView($id) {
        ...
        $sql = "SELECT * FROM category WHERE customers_id = '$id' $searchtxt ORDER BY id DESC ";
        $count = Yii::app()->db->createCommand($sql)->query()->rowCount;
        $dataProviderCategory = new CSqlDataProvider($sql, array(
        ...
        $sql = "SELECT * FROM project WHERE customers_id = '$id' $searchtxt ORDER BY id DESC ";
        $count = Yii::app()->db->createCommand($sql)->query()->rowCount;
        $dataProviderProject = new CSqlDataProvider($sql, array(
            'totalItemCount' => $count,
            'pagination' => array(
                'pageSize' => $pages
            ),
        ...
        $sql = "SELECT * FROM sku WHERE customers_id = '$id' $searchtxt ORDER BY id DESC ";
        $count = Yii::app()->db->createCommand($sql)->query()->rowCount;
        $dataProviderSku = new CSqlDataProvider($sql, array(
            'totalItemCount' => $count,
            'pagination' => array(
                'pageSize' => $pages
            ),
        ...
        $this->render('view', array(
            ...
            'dataProviderSku' => $dataProviderSku,
            'dataProviderProject' => $dataProviderProject
        ));
    }
    ...
    public function actionCreate() {
        try {
            ...
            //SET Permsion
            $access = new UserHasCustomers();
            $access->customers_id = $model->id;
            $access->users_id = Yii::app()->user->getId();
            $access->online = 1;
            $access->save();
    ...
    public function actionIndex() {
        ...
        $sql = "SELECT * FROM customers WHERE id IN ($cus_keys) $searchtxt ORDER BY id DESC ";
        $count = Yii::app()->db->createCommand($sql)->query()->rowCount;
